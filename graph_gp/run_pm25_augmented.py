"""PM2.5 lgamma_block with augmented K-means: features = [lat_std, lon_std, w * d_i_std].

One (K, w, seed) per job. K-means on (lat, lon, d_i); per-cluster k-NN
graph uses geographic (lat, lon) only; lgamma_block fits K gamma_k via
MLL.

Usage:
    python graph_gp/run_pm25_augmented.py --K 5 --w 0.5 --seed 1729
"""
import argparse
import json
import os
...
from run_all_methods import (
    fit_het_joint, learn_gamma_per_cluster, load_pm25,
)
...
if __name__ == '__main__':
    ...
    A, L, n, y, d_raw, coords_km, _ = load_pm25()
    ...
    print(f'pm25 aug K={args.K} w={args.w} seed={args.seed} n={n}', flush=True)
    ...
    if params is None:
        result = {
            'seed': args.seed, 'dataset': 'pm25_aug', 'K': args.K,
            'w': args.w, 'n': int(n),
        ...
        result = {
            'seed': args.seed, 'dataset': 'pm25_aug',
    ...
    out = f'{args.outdir}/results_pm25_aug_K{args.K}_w{args.w:.1f}_seed{args.seed}.json'
